...
func TestNewStep(t *testing.T) {
	t.Run("valid step", func(t *testing.T) {
		step, err := NewStep(types.Step{Function: "set-labels", Target: types.Target{Kind: "Deployment"}, Config: map[string]any{"foo": "bar"}})
		...
		assert.Equal(t, map[string]any{"foo": "bar"}, step.Config)
	})

	t.Run("invalid step with non-existent function", func(t *testing.T) {
		step, err := NewStep(types.Step{Function: "zzz", Target: types.Target{Kind: "Deployment"}, Config: map[string]any{"foo": "bar"}})
		assert.Error(t, err)
		assert.Nil(t, step)
	})

	t.Run("invalid step with invalid target", func(t *testing.T) {
		step, err := NewStep(types.Step{Function: "set-labels", Target: types.Target{APIVersion: "apps/v1", Group: "apps"}, Config: map[string]any{"foo": "bar"}})
		assert.Error(t, err)
		assert.Nil(t, step)
	})
}
...
func TestStepRun(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			step:    &types.Step{Function: "set-labels", Target: types.Target{Kind: "Deployment"}, Config: map[string]any{"foo": "bar2"}},
		...
		},
		{
			...
			step:    &types.Step{Function: "set-labels", Target: types.Target{Kind: "Service"}, Config: map[string]any{"foo": "bar2"}},
		...
		{
			name:    "function fails",
			step:    &types.Step{Function: "run-fail", Target: types.Target{Kind: "Deployment"}, Config: map[string]any{"foo": "bar"}},
			want:    &unstructured.Unstructured{Object: map[string]any{"apiVersion": "apps/v1", "kind": "Deployment", "metadata": map[string]any{"labels": map[string]any{"foo": "bar"}}}},
			wantErr: true,
		},
	}
